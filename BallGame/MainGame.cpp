...
struct BallSpawn {
    BallSpawn(const Gengine::ColorRGBA8& colr,
...
#define ADD_BALL(p, ...) \
    ...
    for (int i = 0; i < NUM_BALLS; i++) {
        ...
        m_balls.emplace_back(ballToSpawn->radius, ballToSpawn->mass, pos, direction * ballToSpawn->randSpeed(randomEngine),
            Gengine::ResourceManager::getTexture("Textures/circle.png").id,
            ballToSpawn->color);
...
void MainGame::draw() {
    ...
    m_ballRenderers[m_currentRendererIdx]->renderBalls(m_spriteBatch, m_balls, projectionMatrix);
...
void MainGame::processInput() {
    ...
    if (m_inputManager.isKeyPressed(SDLK_r)) {
        m_currentRendererIdx++;
        if (m_currentRendererIdx >= (int)m_ballRenderers.size()) {
            m_currentRendererIdx = 0;
        }
    }
}
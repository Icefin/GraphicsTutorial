...
void Grid::addBall(Ball* ball) {
    ...
    ball->ownerCell = cell;
    ball->cellVectorIdx = cell->balls.size() - 1;
}
...
void Grid::removeBallFromCell(Ball* ball) {
    ...
    // Normal vector swap
    balls[ball->cellVectorIdx] = balls.back();
    balls.pop_back();
    // Update vector index
    if ((size_t)ball->cellVectorIdx < balls.size()) {
        balls[ball->cellVectorIdx]->cellVectorIdx = ball->cellVectorIdx;
    }
    // Set the index of ball to -1
    ball->cellVectorIdx = -1;
